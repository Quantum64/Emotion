...
public class StringOpcodes extends OpcodeRegistry {
	...
	public void register() {
		...
		r("string.format", stack -> stack.push(String.format(stack.peek(2).toString(), stack.pull(2).iterate().toArray())), "Push the second stack value formatted with arguments from the first stack value.");
		...
		r("string.toCharArray", stack -> stack.push(Arrays.asList(stack.pop().toString().toCharArray())), "Push a list of characters that make up the first stack value.");
		...
		r("string.unique", stack -> stack.push(unique(stack.pop().toString())), "Push the first stack value with duplicate characters removed.");
		...
		r("string.lowerCamelCase", stack -> stack.push(lowerCammelCasr(stack.pop().toString())), "Push the first stack value transformed to lowerCamelCase.");
		r("string.camelCase", stack -> stack.push(camelCase(stack.pop().toString())), "Push the first stack value transformed to CamelCase.");
		r("string.sentenceCase", stack -> stack.push(sentenceCase(stack.pop().toString())), "Push the first stack value transformed to Sentence case.");
		r("string.shiftChars", stack -> stack.push(shiftChars(stack.pop().toString(), stack.pop().asInt())), "Push the second stack value with all characters arithmetically shifted by the first stack value.");
		...
		r("string.padStart", stack -> stack.push(Collections.nCopies(stack.pop().asInt(), " ").stream().collect(Collectors.joining()) + stack.pop()), "Push the second stack value padded with the fist stack value number of spaces on the left.");
		r("string.padEnd", stack -> stack.push(stack.peek(2) + Collections.nCopies(stack.pull(2).asInt(), " ").stream().collect(Collectors.joining())), "Push the second stack value padded with the first stack value number of spaces on the right.");
		r("string.middle", stack -> stack.push(stack.peek().toString().length() == 0 ? "" : stack.peek().toString().charAt((stack.pop().toString().length() - 1) / 2)), "Push the middle character in the first stack value.");
		
	}

	private String deleteEnd(String target, String str) {
	...
	private String lowerCammelCasr(String str) {
		char[] chars = str.toCharArray();
		StringBuilder result = new StringBuilder();
		for (int i = 0; i < chars.length; i++) {
			result.append(i != 0 && String.valueOf(chars[i - 1]).equals(" ") ? String.valueOf(chars[i]).toUpperCase() : String.valueOf(chars[i]).toLowerCase());
	...
	private String shiftChars(String str, int amount) {
		StringBuilder result = new StringBuilder();
