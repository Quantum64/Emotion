...
public class Smaz {
	...
	public byte[] compress(String inString) {
		...
		CharBuffer charBuffer = new CharBuffer(inString);
		int inlen;
		...
		while ((inlen = charBuffer.remaining()) > 0) {
			...
			for (; j > 0; j--) {
				CharBuffer slot;
				if (j == 1) {
					slot = new CharBuffer(CODEBOOK[h1 % 241]);
				} else if (j == 2) {
					slot = new CharBuffer(CODEBOOK[h2 % 241]);
				} else {
					slot = new CharBuffer(CODEBOOK[h3 % 241]);
				}
				...
				while (slotLength > 0 && slotEndIndex <= slotLength) {
					if (slot.get(slotIndex) == j && inlen >= j && slot.subSequence(slotIndex + 1, slotEndIndex).toString().equals(charBuffer.subSequence(0, j).toString())) {
	...
	public boolean canCompress(String input) {
		char[] chars = input.toCharArray();
		for (char c : chars) {
			if (c > 127)
				return false;
